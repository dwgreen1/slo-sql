-- SENT INN-REACH IN-TRANST TOO LONG FROM WCPL
SELECT i.barcode, b.title,
	CASE WHEN i.vol IS NULL THEN c.content ELSE c.content||' '||i.vol END call_num,
		i.price, i.in_transit_date, i.due_date, i.elapsed,
		ll.name AS home_loc, bl.name AS desintation, i.pickup, i.patron
FROM
	(SELECT ip,barcode, m.field_content, to_char(ch.due_gmt,'MM-DD-YYYY') AS due_date,
	 ch.checkout_gmt, to_char(ch.checkout_gmt,'MM-DD-YYYY') AS in_transit_date, ip.call_number,
	 (to_char(ch.due_gmt,'MM-DD-YYYY')::timestamp - to_char(ch.checkout_gmt,'MM-DD-YYYY')::timestamp) AS elapsed,
...
WHERE i.checkout_gmt < '2022-02-09'
-- THE NEXT TWO LINES DETERMINE THAT THE ITEM WAS SENT BUT HAD NOT YET ARRIVED TO BORROWING LIBRARY.
-- A VALUE OF 7 OR 21 DAYS FOR I.ELAPSED ARE BASED ON THE DEFAULT DUE DATE MINUS THE CHECKOUT/TRANSIT DATE.
-- ONCE THE ITEM IS RECEVIED, THE DUE DATE IS UPDATED, CHANGING THE I.ELAPSED VALUE TO BE GREATER THAN 7 OR 21.
-- IF I.ELAPSED IS STILL 7 OR 21 DAYS AFTER A MONTH OR SO FROM THE TRANSIT DATE, IT IS LIKELY THE ITEM WAS LOST IN TRANSIT AND SHOULD BE CLAIMED.
AND i.field_content LIKE '%requested by%'
AND (i.elapsed = '7 days' OR i.elapsed = '21 days')
AND i.elapsed > '7 days'
